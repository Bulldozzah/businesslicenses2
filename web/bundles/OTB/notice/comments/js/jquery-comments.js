...
(function (factory) {
    ...
    var Comments = {
        ...
        events: {
            ...
            'click.li.comment span.socialShare': 'shareComment',
            'click li.comment button.delete.enabled': 'deleteComment',
            'click li.comment .hashtag': 'hashtagClicked',
            'click li.comment .ping': 'pingClicked',
            'click li.comment button.abusive-action': 'markAbusive',
        ...
        getDefaultOptions: function () {
            return {
                ...
                oldestText: 'Oldest',
                popularText: 'Popular',
                adminUpvoted: 'Upvoted',
                attachmentsText: 'Attachments',
                sendText: 'Send',
                replyText: 'Reply',
                markAbusiveText: 'Report Abusive',
                shareText: 'Share',
                editText: 'Edit',
                editedText: 'Edited',
                youText: 'You',
                ...
                readOnly: false,
                //defaultNavigationSortKey: 'adminUpvoteCount',
                defaultNavigationSortKey: 'newest',
                includeAttachmentInPost: true,
                ...
                fieldMappings: {
                    ...
                    markAbusive: 'marked_abusive',
                    hidden: 'hidden',
                    userIsLoggedIn: 'user_logged_in',
                    adminUpvoted: 'admin_upvote_count',
                ...
                markAbusive: function (commentJSON, success, error) { success(commentJSON) },
                shareComment: function (commentJSON, success, error) { success(commentJSON) },
                hashtagClicked: function (hashtag) { },
                pingClicked: function (userId) { },
                uploadAttachments: function (commentArray, success, error) { success(commentArray) },
                refresh: function () { },
                timeFormatter: function (time) {
                    date = (time instanceof Date ? time : new Date(Date.parse(time)));
        ...
        sortComments: function (comments, sortKey) {
            ...
            if (sortKey == 'popularity') {
                ...
                // Sort by date
            } else if (-1 < ['adminUpvoted'].indexOf(sortKey)) {
                sortComments = comments.sort(function (commentA, commentB) {
                    return commentB[sortKey] - commentA[sortKey];
                });
            } else {
                comments.sort(function (commentA, commentB) {
                    var createdA = new Date(commentA.created).getTime();
        ...
        postComment: function (ev) {
            ...
            if (attachment.length > 1) {
                $(".comment-error-section").html(`<p style="color: red;">Your upload is limited to one file</p>`);
                return;
            }
            $(".comment-error-section").html('');
            ...
            let content = this.getTextareaContent(textarea).split(' ').length;

            if (content > 300) {
                $(".comment-error-section").html(`<p style="color: red;">You are limited to 300 words.</p>`);
                return;
            }
            ...
            if (this.options.includeAttachmentInPost) {
                ...
                if (attachmentJSON != null || attachmentJSON != undefined) {
                    commentJSON.file = attachmentJSON.file;
        ...
        upvoteComment: function (ev) {
            ...
            if (this.options.enableEditing) {
                ...
                $(".comment-error-section").html(`<p style="color: red;">Please login to like a comment.</p>`);
                return;
            }
            ...
            if (this.options.enableEditing) {
                ...
                alert("you need to login to like a comment");
                $(".comment-error-section").html(`<p style="color: red;">You need to login to like a comment</p>`);
                return;
            }


        },
        markAbusive: function (ev) {
            var self = this;
            var commentEl = $(ev.currentTarget).parents('li.comment').first();
            var commentModel = commentEl.data().model;
            var commentUrl = "/index.php/comments/api/" + commentModel.id + "/mark-abusive";

            $.getJSON(commentUrl);
            commentModel.markAbusive = true;
            //this.renderMarkAbusive(commentModel.id);
            //alert('Comment Marked as abusive. Awaiting Admin Review');
            var commentJSON = $.extend({}, commentModel);
            commentJSON = this.applyExternalMappings(commentJSON);
            ...
            var success = function (commentJSON) {
                var commentModel = self.createCommentModel(commentJSON);
                // self.updateCommentModel(commentModel);
                self.renderMarkAbusive(commentModel.id);
            };
            ...
            this.options.markAbusive(commentJSON, success, error);
        },
        shareComment: function (ev) {
            var self = this;
            var commentEl = $(ev.currentTarget).parents('li.comment').first();
            var commentModel = commentEl.data().model;
            var commentJSON = $.extend({}, commentModel);
            ...
            var success = function (commentJSON) {
                var commentModel = self.createCommentModel(commentJSON);
                self.updateCommentModel(commentModel);
                self.renderShareComment(commentModel.id);
            };
            ...
            this.options.shareComment(commentJSON, success, error);
        },
        displayShareButtons: function (url, options) {
        ...
        replyButtonClicked: function (ev) {
            ...
            if (previousParentId != parentId) {
                ...
                // var anchor1 = document.getElementById('textarea-wrapper_reply');
                // document.getElementById(`textarea-wrapper-reply`).scrollIntoView( {behavior: "smooth" })
            }
        },
        ...
        createNavigationElement: function () {
            ...
            var adminUpvoted = $('<li/>', {
                text: this.options.textFormatter(this.options.adminUpvoted),
                'data-sort-key': 'adminUpvoted',
                'data-container-name': 'comments'
            });
            ...
            navigationWrapper.append(adminUpvoted);
            dropdownNavigation.append(adminUpvoted.clone());
        ...
        createCommentWrapperElement: function (commentModel) {
            if (commentModel.hidden) {
                var commentWrapper = $('<div/>', {
                    'class': 'comment-wrapper hidden'
                });
            } else {
                var commentWrapper = $('<div/>', {
            ...
            // Mark Abusive
            if (commentModel.hidden || commentModel.createdByCurrentUser) {
                var markAbusiveButton = '';
            } else {
                var markAbusiveButton = $('<button/>', {
                    'class': 'actions abusive-action',
                    'type': 'button',
                    text: this.options.textFormatter(this.options.markAbusiveText)
                });
            }
            ...
            if (commentModel.hidden) {
                ...
                var shareButton = `
	            	<div class="action reply show-media-icon">
                        <div class="share_title">Share: </div>
                        <span style="separator"><a target="_blank" class="share_fb" href="https://www.facebook.com/sharer/sharer.php?u=${location.protocol + '//' + location.host + location.pathname}&text=${commentModel.content}"><ion-icon name="logo-facebook"></ion-icon></a></span>
		            	<span style="separator"><a target="_blank" class="share_twttr" href="https://twitter.com/intent/tweet?url=${location.protocol + '//' + location.host + location.pathname}&text=${commentModel.content}"><ion-icon name="logo-twitter"></ion-icon></a></span>
		            	<span style="separator"><a target="_blank" class="share_mail" href="mailto:?Subject=${location.protocol + '//' + location.host + location.pathname}&body=${commentModel.content}" target="_blank"><ion-icon name="mail"></ion-icon></a></span>
		            	`;
                var isMobile = {
                    Android: function () {
                ...
                if (isMobile.any()) {
                    shareButton += `<span style="separator"><a target="_blank" class="share_wtsp" href="whatsapp://send?text=${encodeURIComponent(commentModel.content + '  ' + location.protocol + '//' + location.host + location.pathname)}" data-action="share/whatsapp/share"><ion-icon name="logo-whatsapp"></ion-icon></a></span>
                                    </div>`;
                } else {
                    shareButton += `<span style="separator"><a target="_blank" class="share_wtsp" href="https://web.whatsapp.com/send?l=en&text=${encodeURIComponent(commentModel.content + '  ' + location.protocol + '//' + location.host + location.pathname)}" data-action="share/whatsapp/share"><ion-icon name="logo-whatsapp"></ion-icon></a></span>
                                    </div>`
                }
            }
            ...
            /**$('<span/>', {
                ...
                'class': 'showSocialButtons fa fa-share-alt'
            })).append($('<span/>', {
                'class': 'socials socials' + commentModel.id
            })); **/
            ...
            if (commentModel.markAbusive && this.options.enableMarkAbusive) {
                actions.append(this.createMarkAbusiveElement(commentModel));
            } else if (this.options.enableMarkAbusive) {
                actions.append(markAbusiveButton);
            }
            ...
            if (this.options.enableShare) actions.append(shareButton);
        ...
        createMarkAbusiveElement: function (commentModel) {

            // makr abusive
            var markedAbusiveEl = $('<button/>', {
                'class': 'action abusive ' + (commentModel.markAbusive ? 'highlight-font' : '')
            }).append($('<span/>', {
                text: "REPORTED ABUSIVE",
                'class': 'marked-abusive'
            }));

            return markedAbusiveEl;
        },
        /**createShareElement: function (commentModel) {
            var shareElement = $('<span/>', {
                'class': 'actions socialShare socialPlugin',
            }).append($('<span/>', {
                'class': 'showSocialButtons fa fa-share-alt'
            })).append($('<span/>', {
                'class': 'socials'
        ...
        renderMarkAbusive: function (id) {
            var commentModel = this.commentsById[id];
            var commentElements = this.$el.find('li.comment[data-id="' + commentModel.id + '"]');

            var self = this;
            commentElements.each(function (index, commentEl) {
                var markabusive = self.createMarkAbusiveElement(commentModel);
                $(commentEl).find('.abusive-action').first().replaceWith(markabusive);
            });
        },
        renderShareComment: function (id) {

            var commentModel = this.commentsById[id];
            var commentElements = $("#comment-list").find('li.comment[data-id="' + commentModel.id + '"]');
            // var commentElements = this.$el.find('li.comment[data-id="' + commentModel.id + '"]');
