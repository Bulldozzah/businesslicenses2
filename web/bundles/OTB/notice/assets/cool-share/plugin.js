(function ($) {
	...
	$.fn.shareButtons = function (url, options, span_class) {
		...
		if (options.whastapp.text) {
			shareURLs['whatsapp'] += '&text=' + encodeURIComponent(options.whastapp.text);
		}
		...
		this.each(function(i){
			...
			var socialButtonsPopUp = elem.find('.socials' + span_class);
		...
		var socials = $('.socialPlugin .socials' + span_class),
		...
		/**$('.showSocialButtons').click(function() {
			$(this).parent().find('.socials' + span_class).toggleClass('opened');
		});**/
		...
		$(document).off('.social-plugin').on('click.social-plugin touchstart.social-plugin', function(e){
			if(!$(e.target).hasClass('showSocialButtons') && !$(e.target).parent().hasClass('socials' + span_class)) {
